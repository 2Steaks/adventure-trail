...
- **Every module PR must be:**
  - **Described** — never an empty/title-only PR. The description states what shipped, what's explicitly not in this PR yet (if the module isn't fully done), and a test plan checklist. Non-negotiable even for small or WIP PRs.
