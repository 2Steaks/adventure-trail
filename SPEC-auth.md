...
- **Proxy matcher regression test**: assert `/` requires auth while `/login` and `/register` don't, using Next's `unstable_doesProxyMatch` (`next/experimental/testing/server`, stable since 15.1). This is the regression test for the bug found above — write it first against the *current* (broken) state to confirm it fails, then fix the location/matcher, confirm it passes.
