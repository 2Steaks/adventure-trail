...
**Acceptance criteria:**
- [ ] `getCurrentPosition()` lives in `src/lib/geo/geolocation.ts`, exported
- [ ] `src/lib/places/hooks.ts` imports it from the new location; no duplicate definition remains
- [ ] `useNearbyPlaces()`'s behavior is unchanged (still a 10s timeout, still the same error messages)
