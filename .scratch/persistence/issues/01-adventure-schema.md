...
**Acceptance criteria:**
- [x] Valid body (theme + in-range ages + an allowed `durationMinutes`/`maxDistanceMeters`) passes
- [x] `ageMax < ageMin` fails
- [x] A `durationMinutes` or `maxDistanceMeters` value outside the allowed set (`30/60/90/120` and `500/1000/2000/5000` respectively) fails
- [x] Test written first, observed failing (import error, schema didn't exist) before the schema existed
- [x] `HARD_CODED_QUEST` exported with `objective`, `type`, `landmarkName`, `landmarkType`, `latitude`, `longitude`, `radiusMeters` matching `SPEC-persistence.md`
