...
- [ ] Returns a `400`/`409`-class error with no LLM call when the adventure has no current quest (already fully completed)
- [ ] A validated `COMPLETE_OBJECTIVE` completes the quest, advances `game_states.current_quest_id` to the next pending quest by position, and completes the adventure when it was the last quest
- [ ] A validated `ADD_ITEM` appends to `game_states.inventory` without clobbering existing items
- [ ] An `EncounterError` (retry exhausted) returns `502` with zero DB writes (quest/game-state/message all unchanged)
- [ ] Calling the route again after a quest is already completed is idempotent
- [ ] `/adventures/[id]` derives the current quest from `gameState`, renders the Wizard through all six mapped states, and shows `EncounterPanel` with tappable choices once an encounter has run
- [ ] `HARD_CODED_QUEST`-era assumptions (single-quest-per-adventure UI) are fully gone from the detail page

**Verification:**
- [ ] `pnpm build`/`lint`/`test` pass
- [ ] Manual, no live LLM call needed: unauthenticated → `307`; wrong owner → `404`; adventure with no current quest → clean `400`/`409`, zero writes
- [ ] Manual, deliberately bounded (real billed API call, contingent on task 04's live verification not being deferred): one full encounter — arrive → talk to Wizard → AI message + choices rendered → objective completed → `game_states.current_quest_id` advanced (or adventure completed on the final quest) — confirmed via direct DB query
- [ ] Every `SPEC-ai-encounter.md` Success Criteria box checked (or explicitly marked deferred, matching task 04's live-verification status)
